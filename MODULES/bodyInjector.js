...
module.exports = {
    ...
    loadModule: async (id) => {
        ...
        iframe.src = `https://${cache.sdk.beta ? "beta." : ""}slive.app/modules/${id}?${Date.now()}&` + utils.jsonToUrlParams(utils.getUrlVars())
        if(cache.sdk.local) iframe.src = `http://localhost:5500/SDK/overlay/module/index.html?${Date.now()}`
